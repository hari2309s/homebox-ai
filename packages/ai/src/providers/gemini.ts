...
export function createGeminiModel(model = process.env.GOOGLE_MODEL ?? "gemini-2.5-flash") {
  ...
  // Gemini 2.5+ models have thinking enabled by default, which can push
  // vision/structured-output calls past Vercel's 60 s function limit.
  return new ChatGoogleGenerativeAI({ apiKey, model, thinkingConfig: { thinkingBudget: 0 } });
}
